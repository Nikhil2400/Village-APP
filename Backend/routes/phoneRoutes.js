...
router.post('/add-number', (req, res) => {
  ...
  db.query(sql, [name, number], (err) => {
    if (err) return res.status(500).json({ success: false, message: 'Failed to add number', error: err.message });
...
router.get('/get-numbers', (req, res) => {
  ...
  db.query(sql, (err, results) => {
    if (err) return res.status(500).json({ success: false, message: 'Failed to retrieve numbers', error: err.message });
...
router.delete('/delete-number/:id', (req, res) => {
  ...
  db.query(sql, [id], (err) => {
    if (err) return res.status(500).json({ success: false, message: 'Failed to delete number', error: err.message });
...
router.post('/api/send-message', async (req, res) => {
  ...
  db.query(sql, numbers, async (err, results) => {
    if (err) return res.status(500).json({ success: false, message: 'Failed to fetch recipients', error: err.message });
...
router.put('/update-message/:id', (req, res) => {
  ...
  if (!message) {
    return res.status(400).json({ success: false, message: 'Message is required for update' });
  }
